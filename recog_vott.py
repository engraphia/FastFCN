...
model = get_segmentation_model('fcn', dataset='vott',
    lateral=False, jpu=True,
    pretrained=False,
    )

# モデルに学習結果を読み込む
checkpoint = torch.load(
   os.getcwd() + '/experiments/segmentation/runs/vott/fcn/vott/model_best.pth.tar'
   )
...
from encoding.datasets import VOTTSegmentation
dataset = VOTTSegmentation()
palette = dataset._generate_mask_palette(dataset.json['tags'])
